...
def game_state(db: Session, current_user: User, attempt: WordleAttempt, puzzle_date: date) -> GameStateRead:
    ...
    return GameStateRead(
        ...
        answer_pool_count=len(COMMON_ANSWER_WORDS),
...
def answer_for_date(puzzle_date: date) -> str:
    ...
    return COMMON_ANSWER_WORDS[int(digest, 16) % len(COMMON_ANSWER_WORDS)]
...
def word_pool() -> set[str]:
    pool = list(COMMON_ANSWER_WORDS)
    seen = set(pool)
    for word in system_dictionary_words():
        if word in seen:
            continue
        seen.add(word)
        pool.append(word)
