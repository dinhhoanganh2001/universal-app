...
- The frontend has an in-app feature notification system. New feature announcements live in `featureAnnouncements` in `src/app.js`, show once automatically after login/onboarding until acknowledged, remain accessible from the sidebar `Thông báo` button, and persist read state in localStorage.
...
- Future user-facing feature work should add a new `featureAnnouncements` entry in `src/app.js` with a stable id, Vietnamese copy, purpose, recommended use cases, how-to steps, and an optional CTA module target.
