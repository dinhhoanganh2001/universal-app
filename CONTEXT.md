...
- Root and backend env examples also use `ACCESS_TOKEN_EXPIRE_MINUTES=43200`; if users are logged out after about an hour, check ignored runtime `.env` files for an old `ACCESS_TOKEN_EXPIRE_MINUTES=60` override and restart the backend after changing it.
...
- The budget-add dropdown and transaction-add dropdown use the category list defined on the `Danh mục` page; historical categories may still appear in filters so old data remains searchable. The transaction-add `Danh mục` dropdown sorts defined categories by that user's transaction usage count, with unused categories falling back to the defined category order. The budget-add control is collapsed to a single add button by default, expands into category and amount fields on click, shows the full defined category list, and blocks duplicate current-month budgets with a toast.
- Users can manage friends on the `Bạn bè` page and see only each friend's current-month total budget progress percentage; the page also shows the current user as a server-backed, non-removable comparison row. Friend lists and request lists are deduped by user id so reciprocal/legacy duplicate friendship rows do not render twice.
...
- First-time login shows an onboarding screen that asks currency unit, monthly income, and two budget levels per category: `Chi tiêu tối thiểu` and `Chi tiêu đầy đủ`; users can skip with `Thiết lập sau` or rerun it from `Hồ sơ`. Registration auto-login is allowed, onboarding only appears for an authenticated user, and first-time onboarding money fields intentionally start blank. When users choose `Thiết lập ngân sách lại` from `Hồ sơ`, onboarding is prefilled from the current saved bucket, custom funds, monthly income, currency, and current-month effective budgets so they can edit instead of remembering previous settings. The onboarding screen shows financial independence/freedom timeline estimates directly while the user enters values.
...
- The budget dashboard estimates time to `Độc lập tài chính` from 25 years of `Chi tiêu tối thiểu`, and `Tự do tài chính` from 25 years of `Chi tiêu đầy đủ`, using monthly income minus the matching budget level as monthly savings and subtracting current `Túi tiền` from the target before calculating remaining time.
...
- The app brand mark uses `src/assets/app-logo.jpg` in the sidebar, auth showcase, and browser favicon.
- Phone layouts are tightened across auth, onboarding, navigation, transaction review, budgets, funds, friends, profile, game progress, and feature notifications. At 700px and below, the sidebar becomes a compact sticky header with horizontally scrollable modules, transaction tables become labeled cards, onboarding edit rows stack, and the feature notification modal behaves like a bottom sheet. iPhone Safari is handled with viewport-fit, dynamic viewport height, text-size-adjust, viewport width clamps, and bottom safe-area padding so browser chrome does not hide the main content.
...
- The money app now separates monthly budgets from long-term storage goals: `Túi tiền` records total current money, custom `Quỹ` records store user-created saved/target goals, and the strategic funds `Quỹ khẩn cấp`, `Quỹ độc lập tài chính`, and `Quỹ tự do tài chính` are calculated automatically from budget levels instead of being stored as editable funds.
- The frontend has an in-app feature notification system. New feature announcements live in `featureAnnouncements` in `src/app.js`, show once automatically after login/onboarding until acknowledged, remain accessible from the sidebar `Thông báo` button, and persist read state in localStorage. The notification list sorts by exact timestamp newest-first, shows compact clickable summaries with exact `+07` times, and clicking a summary opens a dedicated full detail view for purpose/recommendation/how-to content.
...
- Wordle has an on-screen virtual keyboard for mouse input; keys are colored by the best known result for each letter.
- Wordle guesses are validated against committed SCOWL-derived 5-letter English word data with US and British spellings. Hidden answers are a separate committed list generated from the top 3000 wordfreq English words, filtered to SCOWL-valid base nouns, base verbs, or adjectives.
...
- `index.html`: static app shell and frontend asset version query strings.
- `src/assets/app-logo.jpg`: current web app logo used for visible brand marks and favicon.
...
- `backend/app/models/money.py`: money transactions, categories, budgets, bucket, and fund persistence models.
- `backend/app/api/v1/game.py`: daily Wordle state, guess submission, scoring, word-pool handling, and player progress reads.
- `backend/app/data/game/answer_words.txt`: committed hidden Wordle answer list generated from common English words with noun/verb/adjective filtering.
- `backend/app/data/game/valid_guesses.txt`: committed accepted Wordle guess list generated from SCOWL US/British spelling data.
- `backend/app/data/game/SOURCES.md`: source notes for generated GAME word data.
...
- `backend/scripts/generate_game_word_lists.py`: optional generator for refreshing committed GAME word lists from SCOWL, wordfreq, and WordNet.
...
- Friend progress intentionally exposes only current-month aggregate percentage and budget count, not friend money amounts; pending friend requests expose identity/status only.
- `Túi tiền` is a single current-total snapshot per user; custom `Quỹ` records are independent long-term targets and do not currently enforce that saved amounts sum to the bucket total.
- Strategic funds are reserved names, not user-editable records: `Quỹ khẩn cấp` target is 12 months of `Chi tiêu tối thiểu`, `Quỹ độc lập tài chính` target is 25 years of `Chi tiêu tối thiểu`, and `Quỹ tự do tài chính` target is 25 years of `Chi tiêu đầy đủ`. The frontend shows them as read-only automatic fund cards using current `Túi tiền` as progress, and timeline estimates use current `Túi tiền` to reduce the remaining target before calculating time. The backend rejects manual creation/update to those reserved fund names.
- Future user-facing feature work should append a new `featureAnnouncements` entry in `src/app.js` with a stable id, exact timestamp/time label, Vietnamese copy, purpose, recommended use cases, how-to steps, and an optional CTA module target; the UI sorts entries by timestamp so the newest notification appears first.
...
- Bucket/custom-fund tracking does not yet support transaction history, automatic allocations, or transfers between funds; values are manually maintained snapshots. The three strategic funds are target calculations only and do not create allocation transactions.
- There is no automated frontend browser test yet.
- There is no automated mobile visual regression test yet; current phone UI improvements are CSS-based and covered by syntax/backend smoke checks only.
...
- The Wordle answer list is generated from the top 3000 wordfreq English words, but only 5-letter SCOWL-valid base nouns, base verbs, and adjectives become hidden answers. Accepted guesses use a broader SCOWL 2020.12.07 derived list for US/British spelling variants.
- Regenerating GAME word data is optional and not needed at runtime. The local generation setup used `backend/.venv/bin/pip install wordfreq nltk`, NLTK `wordnet` and `omw-1.4` corpora, and the SCOWL 2020.12.07 archive, then ran `backend/.venv/bin/python backend/scripts/generate_game_word_lists.py --scowl-archive /path/to/scowl-2020.12.07.tar.gz`.
...
- Added Wordle Phase 2: clickable virtual keyboard with letter colors and broader valid-English-word guess acceptance outside the 3000-word ranked pool.
- Added `Túi tiền` and `Quỹ`: backend bucket/fund APIs and models, a new frontend `Túi tiền` navigation page, onboarding fields for current money and multiple long-term goals, and smoke coverage for bucket/fund CRUD.
- Changed strategic funds to automatic calculations: `Quỹ khẩn cấp` is 12 months of minimum spending, `Quỹ độc lập tài chính` is 25 years of minimum spending, and `Quỹ tự do tài chính` is 25 years of full spending; onboarding and custom fund forms no longer let users create those reserved funds manually.
- Added in-app feature notifications with a detailed first announcement for `Túi tiền` and `Quỹ`, plus a sidebar `Thông báo` button, read-state persistence, and asset cache-busting update.
- Changed `Thiết lập ngân sách lại` to prefill onboarding from current saved settings and added a matching in-app feature notification explaining the improvement.
- Changed feature notifications to show newest announcements first as brief clickable summaries, with full detail opening only after the user clicks `Xem chi tiết`.
- Added a GAME notification based on `task2.txt`, added exact `+07` timestamps to every notification, and changed notification expansion from inline content to a dedicated detail view to avoid clipped expanded cards.
- Changed financial independence/freedom time estimates to subtract current `Túi tiền` from the target, show remaining target, and added a feature notification for the calculation change.
- Changed the `Bạn bè` page to show only current-month budget progress, use server-backed progress for the current user's comparison row, align friend progress with effective budget versions, and added a feature notification for the sync behavior.
- Changed the transaction `Danh mục` dropdown to sort categories by each user's saved transaction usage frequency and added a feature notification for the faster entry flow.
- Improved phone UI responsiveness for auth, onboarding, app navigation, transaction cards, budget/fund/friend/profile/game layouts, and notification modal sizing; updated frontend asset cache-busting to `20260805-mobile-ui`.
- Widened the compact phone responsive breakpoint from 520px to 700px after `bug6.jpg` showed a large phone still receiving tablet navigation and desktop transaction-table layout; updated frontend asset cache-busting to `20260805-mobile-ui-wide-phone`.
- Replaced OS-dependent Wordle word validation with committed word-list data: hidden answers now come from top-3000 common English words filtered to base nouns/base verbs/adjectives, accepted guesses come from SCOWL US/British 5-letter words, and the GAME announcement plus smoke tests were updated.
